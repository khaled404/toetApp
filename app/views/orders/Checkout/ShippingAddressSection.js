...
import { SetShippingAddressId } from '../../../actions/CheckoutActions';
...
class ShippingAddressSection extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (this.state.shipping_address_id != this.props.shipping_address_id) {
            let choosedAddress = this.props.addresses[this.props.shipping_address_id];
            this.setState({
                address: {
                ...
                shipping_address_id: this.props.shipping_address_id
            });
        }
    }
    onEditPress() {
    ...
    onAddressPress(id) {
        ...
        this.props.SetShippingAddressId(id);
    }
    render() {
        let content = null;
        ...
        else if (this.props.shipping_address_id == "0") {
        ...
        return (
            <View style={{ flexDirection: 'row' }}>
                ...
                <TouchableOpacity onPress={this.onEditPress} style={{ flex: 3, justifyContent: 'center', alignItems: 'flex-start' }}>
                    <Text style={styles.SectionTitle}>{TranslateString("text_checkout_3_title")}</Text>
                    {content}
                </TouchableOpacity>
